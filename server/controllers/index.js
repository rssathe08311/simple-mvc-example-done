...
const getNameDog = async (req, res) => {
  try {
    ...
    // If we did get a dog back, store it's name in the name variable.
    if (doc) {
      return res.json({ name: doc.name });
    }
...
const makeDogAge = async (req, res) => {
  if (!req.body.name) {
    return res.status(400).json({ error: 'Name is required to perform a search', query: req.body });
  }

  try {
    // Use findOneAndUpdate to find a dog by name and increment its age
    ...
    // If no dog is found, return an error
    if (!updatedDog) {
      return res.status(404).json({ error: 'Dog does not exist' });
    }

    // If a dog was found and updated, return the updated dog data
    return res.json({ name: updatedDog.name, age: updatedDog.age });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ error: 'Something went wrong' });
  }
